...
for (b in 1:B) {
  ...
  stat_norm[b] <- ks.test(X.star, "pnorm", mean = 0.8515278, sd = 0.304962)$statistic
...
print(descdist(Time_Type2$time, discrete = FALSE,boot = 500)) ## checks skewness and kurtosis 

## some code to test the fit of distribution and find parameters 
fit.gamma <- fitdist(Time_Type2$time, "gamma")
fit.norm <- fitdist(Time_Type2$time, "norm")
...
fg <- fitdist(Time_Type2$time, "gamma")
fln <- fitdist(Time_Type2$time, "norm")
...
plotdist(Time_Type2$time, histo = TRUE, demp = TRUE)              
